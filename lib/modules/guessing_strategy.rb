...
module GuessingStrategy
  ...
  # # Checks if the provided ship is sunk.
  # def ship_sunk?(ship)
  #   ship.sunk? # Returns true if the ship's health has reached zero
  # end

  # Decides the next cell to fire upon based on hit tracking.
  def calculate_next_move
    @target_queue.any? ? @target_queue.shift : @board.random_unfired_coordinate
  end
  ...
  def update_strategy(coordinate, hit)
    ...
    # Clears target queue if the hit ship is sunk
    if hit && hit.sunk?
      puts "Debug: Ship #{hit.name} has sunk. Clearing target queue."
      @target_queue.clear
    end
  end
end
  ...
  def add_adjacent_cells_to_queue(coordinate)
    ...
    adjacent_cells.each do |cell|
      if @board.valid_coordinate?(cell) && !@board.cells[cell].fired_upon?
        @target_queue << cell
      end
    end
  end
